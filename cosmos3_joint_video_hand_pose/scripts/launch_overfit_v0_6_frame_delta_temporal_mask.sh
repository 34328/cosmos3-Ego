...
readonly JOB_NAME=overfit_v0.6_frame_delta_temporal_mask
...
readonly CONFIG="$ROOT/configs/overfit_v0_6_frame_delta_temporal_mask.toml"
...
echo "=== $JOB_NAME: B3 + A_t to V_<=t,A_<=t, CP1/FSDP-8/75K ==="
exec "$TORCHRUN" --nproc-per-node=8 --master-port="${MASTER_PORT:-29570}" \
